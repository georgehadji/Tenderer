...
| (audit) | `add_deadline`, `acknowledge`, `close` and every send attempt write an audit event; the deadline screens are `AuditedAdmin`; `send_outbox` runs in one correlation scope. |
