...
| File | Contains / does |
|---|---|
| `__init__.py` | `PACK`: resource kinds `vehicle`, `driver`, `escort` as JSON Schemas (the minimum fields of `docs/architecture.md` §8; `x-personal` marks personal fields, `x-unique` the uniqueness keys), and the sector's document types. Stored attributes are validated against these schemas from M4 (`apps/engagements`). |
