...
| File | Contains / does |
|---|---|
| `__init__.py` | `PACK`: resource kinds `vehicle`, `driver`, `escort` as JSON Schemas (the minimum fields of `docs/architecture.md` §8; `x-personal` marks personal fields, `x-unique` the uniqueness keys), and the sector's document types. `apps/engagements` validates stored attributes against these schemas on every save. The escort keeps only `certificate_valid_until`, the expiry of the medical certificate (AD5). |
