...
| `packs.py` | Pack contracts `SectorPack` and `JurisdictionPack` (frozen dataclasses) and the `Packs` registry. Core never imports a pack; `shell/packs.py` fills the registry and passes it in. |
