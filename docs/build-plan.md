...
| | |
|---|---|
| Status | v1 built: M0–M4 on 2026-09-24, M5–M8 on 2026-09-25 (§2.1, with what only go-live can close). M6 was built on the owner's instruction before the time logs that decide its "Keep if" line. v1.1 onwards waits for its trigger (§1). |
| Date | 2026-09-24 |
...
| | |
|---|---|
| Deliverables | Outbox table with idempotency keys; `send_outbox` and `heartbeat` management commands run by cron (`plan_reminders` deferred, §2.1); e-mail adapter (EU provider, SPF/DKIM/DMARC `p=reject`); `.ics` attachments; escalation ladder T-7/T-3/T-1; 14-day operator dashboard |
...
| Milestone | State on 2026-09-25 | Evidence | Left for later |
|---|---|---|---|
...
| M8 | Built, as far as code and tests reach | Production image and `deploy/smoke.sh` (in CI): migrate, database roles, `check --deploy`, jobs, admin with second factor, image contents; `deploy/restore_drill.sh` run on 2026-09-25 against a seeded PostgreSQL 17 (22 tables, 256 rows, counts equal); `compose.yaml` run (the whole suite passed against it); ASVS L2 self-review (`asvs-l2-review.md`); record of processing and DPIA threshold assessment (`gdpr-v1.md`); `apps/v0import`: a filled v0 workbook imports with zero differences against the oracle that verified the workbook, and one euro of difference rolls it back. Also closed while building M8: an engagement can be opened from the admin; tenders load at runtime with `TENDERER_COMMIT` in their version; a Content Security Policy and Permissions-Policy on every response; a log filter that removes tax ids, phones and e-mails | **Needs the host and real clients, not code:** EU hosting, e-mail provider with SPF/DKIM/DMARC, identity-aware proxy with WebAuthn, heartbeat monitor and the recorded alert drill, cron entries, point-in-time backups and the first monthly restore drill on them, retention N and the legal-basis points marked for the lawyer, assigning roles and enrolling second factors, and the acceptance test itself: one real invitation cycle in v1 in parallel with the v0 workbook (the import makes it one command) |
| M7 | Built | Append-only `audit_event` (trigger refuses UPDATE and DELETE; `deploy/roles.sql` leaves the app role INSERT and SELECT only; both tested); one event per transition, record, deadline, send, export, erasure, view and list, with a correlation id per request and job; roles `operator`, `reviewer`, `admin`; second factor for the admin (TOTP, static break-glass codes); data-subject export and confirmed erasure; `purge_expired` | WebAuthn is at the proxy, not in the app (`architecture.md` §10.2). `purge_expired` refuses to run until the lawyer sets N |
| M6 | Built | Client plan as one HTML e-mail (deadlines, checklist with status and `§`, go/no-go per route with pack labels, draft declarations), previewed and sent from the admin; tender-data version on every plan; R9 draft from `tenders/<id>/declarations/R9.txt` between two watermarks; reviewed snapshot; hostile values escaped (template-injection test) | Sent directly from the admin, not through the outbox: the operator sees a failed send at once. The R9 wording is ours, **UNVERIFIED** against a template the invitation may prescribe. PDF only when a client asks. Plans sent are not recorded until the audit log (M7) |
| M5 | Built | Deadline and outbox rows in one transaction; unique idempotency keys; `send_outbox` and `heartbeat` commands; a failed send retried on the next run and never sent twice (test with a failing mail backend); the heartbeat withheld while any notification waits more than 30 minutes; `.ics` attachment with alarms at T-3 and T-1; T-7/T-3/T-1 reminders; 14-day dashboard red when a send failed, the date passed, or there is no acknowledgement by T-3; snapshot test of the never-ask line (X7) | `plan_reminders` not built: rows are written with the deadline, so there is nothing to plan until deadlines arrive without a transaction (v2 ingestion) or document-expiry reminders are wanted. Deploy items **not done**: EU e-mail provider and SPF/DKIM/DMARC, the external monitor, the cron entry, and the recorded alert drill (all at M8). Two senders at once are kept apart by `SKIP LOCKED`, not tested |
...
| M3 | Built | `Money` rejects floats; `discount_on_reference` price, validator (errors in the workbook's order), guarantees; taxi cost model equal to the Decimal oracle that checked the workbook, for 6 routes × 100 discounts, including the edge route whose break-even is exactly 20 | Mutation baseline recorded on 2026-09-25: 887/1000, then 978 after the tests it prompted; weekly threshold 970 (`scripts/mutation.sh`). `Percent` value object skipped: rates are plain `Decimal` until a second use needs more |
